...
function Native:BuildTabs()
    ...
    for index, tab in ipairs(self.owner.tabs) do
        ...
        local button = self:Button(self.sidebar, label, { Name = tab.id, Position = UDim2.fromOffset(8, 8 + (index - 1) * 48),
...
function Native:Height(model, width)
    local cacheKey = tostring(width) .. "|" .. model.text .. "|" .. model.description .. "|" .. tostring(model.error)
    ...
    local base = ({ Button = 44, Toggle = 48, Slider = 82, Dropdown = 72, TextBox = 76,
        NumberInput = 76, Keybind = 48, ColorPicker = 48, Progress = 70, Section = 40, Separator = 16, Label = 30 })[model.kind] or 48
...
function Native:_renderRows()
    ...
    for _, row in ipairs(rows) do
        ...
        if (Layout.visible(row.y, row.height, offset, viewport) or ownsFocus) and self.owner.visible and not self.minimized then
            ...
            view.root.Position, view.root.Size = UDim2.fromOffset(0, row.y), UDim2.new(1, -5, 0, row.height)
        end
    end
...
local function mount(r, model, scope)
    ...
    local function refresh(reason)
        ...
        if model.kind == "Label" or model.kind == "Paragraph" then
            title.Size = UDim2.new(1, -20, 1, -16 - extra - (model.description ~= "" and 20 or 0))
            description.Position = UDim2.new(0, 10, 1, -20 - extra)
        end
